...
# parser.add_argument('year', action="store", default="1977", help=u'year')
# parser.add_argument('month', action="store", default="8", help=u'month')
# parser.add_argument('day', action="store", default="11", help=u'day')
# parser.add_argument('time', action="store", default="19",help=u'time')    
...
# options = {"year":1977, "month":8, "day":11, "time":19, "start":1850, "end":2030, "b":False, "g":False, "r":False, "n":False}
...
def  main(options):
    ...
    print("-"*120)
    print("墓库：", str(kus).replace("'",""), "解读：（开发中）", end=' ')
    for item in zhus:
        print(''.join(item), end=' ')
    print()
    ...
    print("湿度分数", temps_scores,"正为暖燥，负为寒湿，正常区间[-6,6] 拱：",  get_gong(zhis, gans), "\033[0m")
    ...
    if '劫' in shens2 and  '比' in zhi_shens and '印' in shens2 and not_yang(me):
        print("阴干比劫印齐全，单身，可入道！")
        
    if zhi_shens[0] == '劫' and is_yang(me): 
        print("年阳刃：得不到长辈福；不知足、施恩反怨。")
    if zhi_shens[3] == '劫' and is_yang(me): 
        print("时阳刃：与妻子不和，晚无结果，四柱再有比刃，有疾病与外灾。")
        
    # 阳刃格        
    if zhi_shens[1] == '劫' and is_yang(me):
        all_ges.append('刃')
    ...
    # 自坐阳刃
    if '劫' in zhi_shen3[2] and is_yang(me) and zhis[2] in zhengs:  
    ...
    if zhi_shens[2] == '枭' or zhis.day == xiao_lu:
        ...
        if '枭' in gan_shens and is_yang(me) and zhis.time == me_di:
        ...
        if zhis.count(me_di) > 1 and is_yang(me):
    ...
    if zhis.day == guan_lu or zhi_shens[2] == '官':
        ...
        if is_yang(me) and zhis.time == me_di:
    ...
    if flag:
        ...
        非为吉兆，妇人尤忌之。
        咸池非吉煞，日时与水命遇之尤凶。''')  
        print(taohuas, [zhis.year, zhis.month, zhis.day, zhis.time])
